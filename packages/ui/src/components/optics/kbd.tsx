...
import { useCallback, useRef, useState, useMemo, isValidElement, useEffect } from "react";
 ...
 */
const MODIFIER_KEYS = new Set(["mod", "ctrl", "alt", "shift", "meta", "cmd", "command"]);
...
const kbdVariants = cva("select-none outline-hidden transition-all duration-150", {
  variants: {
    variant: {
      default:
...
function Kbd({
  ...
  const isCompound = useMemo(() => {
    ...
    if (hotkeyProp && typeof hotkeyProp === "string" && hotkeyProp.includes("+")) return true;
    const text = extractTextFromChildren(props.children);
    return text?.includes("+") || (Array.isArray(props.children) && props.children.length > 1);
    ...
    const handleKeyUp = (event) => {
      ...
      if (isOnlyModifier) {
        ...
        if (lastPart === "mod") {
          ...
          isModifierUp = eventKey === "control" || eventCode.includes("control") || !event.ctrlKey;
        } else if (lastPart === "alt") {
          isModifierUp = eventKey === "alt" || eventCode.includes("alt") || !event.altKey;
        } else if (lastPart === "shift") {
          isModifierUp = eventKey === "shift" || eventCode.includes("shift") || !event.shiftKey;
        }
  ...
  return (
    ...
    >
      ...
      >
        <span className="block text-center align-center text-xs">{props.children}</span>
      </span>
    </kbd>
  );
}
