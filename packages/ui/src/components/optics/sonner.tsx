...
import { X, Info, Check, AlertCircle, AlertTriangle, Loader2 } from "lucide-react";
...
export function Toaster({ className = "", ...props }: any) {
    ...
    switch (state) {
      case "success":
        return {
          type: "success",
          title: success,
          description: typeof result === "string" ? result : "Operation completed successfully!",
        };
      case "error":
        return {
          type: "error",
...
function Toast(props) {
  const { title, description, button, id, toastId, type = "info", duration } = props;
  const [isBouncing, setIsBouncing] = React.useState(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Header */}
        <div className={cn("flex flex-col gap-2 text-center sm:text-left relative")}>
          {/* Icon */}
          <div
            className={cn(
            ...
            >
              ...
              >
                {toastType.useSpinner ? (
                  <Spinner
                    size="size-4"
                    className={cn("transition-colors duration-300", toastType.iconColor)}
                  />
                ) : (
                  <IconComponent
                    className={cn("size-4 transition-colors duration-300", toastType.iconColor)}
                  />
                )}
              </div>
              <h2
