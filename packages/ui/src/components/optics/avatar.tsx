...
function Avatar({
  ...
  const renderAvatar = (triggerProps = {}) => {
    const { className: triggerClassName, ref: triggerRef, ...triggerRest } = triggerProps;
...
function AvatarStack({ data = [], children, width = 50, height = 50, alt = "", ...props }: any) {
  return (
    <div className="flex items-center -space-x-2">
      {data.map((item, index) => (
        <Avatar title={item.title} key={index}>
          <AvatarImage src={item.image} width={width} height={height} alt={alt} {...props} />
...
function AvatarImage({ className = "", width = 50, height = 50, alt = "", ...props }: any) {
...
function AvatarGroup({ className = "", ...props }: any) {
  return (
    <div
...
function AvatarGroupCount({ className = "", ...props }: any) {
  return (
    <div
