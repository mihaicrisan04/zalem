...
function DrawerHandle({ className = "", ...props }: any) {
  return (
    <DrawerPrimitive.Handle
      data-slot="drawer-handle"
      className={cn("mx-auto mt-4 h-1.5 w-[100px] rounded-full bg-muted", className)}
      {...props}
    />
  );
}

function DrawerContent({ className = "", children = null, ...props }: any) {
  return (
