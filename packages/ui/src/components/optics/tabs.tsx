...
import {
  Highlight,
  HighlightItem,
} from "@zalem/ui/components/optics/helpers/primitives/effects/highlight";
import { AutoHeight } from "@zalem/ui/components/optics/helpers/primitives/effects/auto-height";
...
const [TabsVariantProvider, useTabsVariant] = getStrictContext("TabsVariantContext");
...
function PrimitiveTabsPanels(props: React.ComponentProps<"div"> = {}) {
  ...
  if (isAutoMode(props)) {
    const { children = null, transition = defaultTransition, ...autoProps } = props;
  ...
  const { children = null, style = {}, transition = defaultTransition, ...layoutProps } = props;
...
function Tabs({ className = "", ...props }: any) {
  return <PrimitiveTabs className={cn("flex flex-col gap-2", className)} {...props} />;
}
...
function TabsList({ className = "", variant = "default", ...props }: any) {
  const highlightStyles = {
    ...
    outline: "absolute z-0 inset-0 border rounded-md dark:border-input dark:bg-input/30",
  ...
  const listStyles = {
    ...
    outline:
      "bg-muted text-muted-foreground inline-flex h-9 w-fit items-center justify-center rounded-lg p-[3px]",
    underline: "inline-flex h-9 w-fit items-center justify-center gap-1 border-b border-border",
  };

  return (
    <TabsVariantProvider value={{ variant }}>
      <PrimitiveTabsHighlight className={highlightStyles[variant]}>
        <PrimitiveTabsList className={cn(listStyles[variant], className)} {...props} />
      </PrimitiveTabsHighlight>
    </TabsVariantProvider>
  );
}

function TabsTab({ className = "", value = "", ...props }: any) {
...
const TabsPanel = ({ className = "", ...props }) => (
  <PrimitiveTabsPanel className={cn("flex-1 outline-none", className)} {...props} />
);
...
export { Tabs, TabsList, TabsTab, TabsTrigger, TabsPanels, TabsContents, TabsPanel, TabsContent };
