...
import { createContext, useContext, useEffect, useState, useLayoutEffect, useRef } from "react";
...
import {
  ...
  SelectTrigger,
  SelectValue,
} from "@zalem/ui/components/optics/select";
import { toast } from "@zalem/ui/components/optics/sonner";
import { ScrollArea, ScrollBar } from "@zalem/ui/components/optics/scroll-area";
...
export const CodeBlockFiles = ({ className = "", children = null, ...props }) => {
...
export const CodeBlockSelectContent = ({ children = null, className = "", ...props }) => {
  const { data } = useContext(CodeBlockContext);
    ...
    <>
      <div className="relative">
        ...
        <div
          className={cn(
            "absolute inset-0 flex items-center justify-center transition-all duration-300 ease-in-out will-change-[transform,opacity,filter]",
            isLoading ? "scale-100 opacity-100 blur-0" : "blur-xs scale-[0.25] opacity-0",
          )}
        >
...
export const CodeBlockContent = ({
  ...
  const shouldCollapse = hasMoreThan10Lines && (fullHeight ? fullHeight > collapsedHeight : true);
