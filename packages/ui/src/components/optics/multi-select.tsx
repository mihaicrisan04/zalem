...
function Select({ onValuesChange = undefined, onOpenChange = undefined, ...props }: any) {
...
function SelectTrigger({
  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon render={<ChevronDownIcon className="size-4 opacity-50 shrink-0" />} />
    </SelectPrimitive.Trigger>
  );
}

function SelectContent({ className = "", children = null, side = "bottom", ...props }: any) {
  return (
...
function SelectItem({ className = "", children = null, value, color = undefined, ...props }: any) {
  const { values, setValue, registerItem, registerColor } = React.useContext(MultiSelectContext);
