...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
      ...
      note/.style={font=\footnotesize\itshape, align=center, text=gray!60!black},
  ...
  \end{tikzpicture}
  \caption{Two-stage recommendation pipeline. The classical
    Stage~1 reads candidate identifiers from the Convex catalog
    under a tight latency budget. Stage~2 invokes the LLM over
    that bounded candidate set only and emits identifiers plus
    a free-text rationale --- it never reads the catalog
    directly. The client then hydrates the cited identifiers
    against the live catalog before rendering, so every field
    the user sees comes from the catalog and not from the LLM.}
...
agree on ordinal --- argues that the harness produces a robust
