\chapter{Zalem: A Case Study}
\label{chap:system}

This chapter presents Zalem, the prototype store built for this
thesis, as a case study of the architecture and process proposed
in Chapters~\ref{chap:intro}--\ref{chap:foundations}. It opens with
...
harness and the measured results across an eight-configuration
sweep. Code excerpts are short and chosen to show a specific
implementation choice; the full source is in the public
repository\footnote{\url{https://github.com/mihaicrisan04/zalem}}
and a live deployment is
available\footnote{\url{https://zalem.mihais-vps.xyz}}.
...
is p95 below 3~s. The 1~s time-to-first-content target is
aspirational because tool-calling depth dominates inference
speed.
...
persistent state lives in Convex tables. The seed pipeline, the
recommendation cron, and the advisor agent all run as Convex
actions, the agent with tool-calling enabled. Convex was chosen
over a conventional REST-plus-SQL-plus-queue stack because the
prototype needs reactive UI updates, scheduled batch flows, and
a shared schema between server and client. Convex provides all
three natively.
...
\code{apps/web} (the Next.js frontend),
\code{packages/backend} (the Convex backend with
  \code{convex/ai/} grouping the advisor, prompts, models,
review summaries, and eval integration),
\code{packages/ui} (shared design system),
\code{packages/eval} (the LLM eval harness),
\code{packages/env} (type-safe environment validation), and
\code{packages/config} (shared TypeScript configuration). The
eval harness depends on the backend, so scoring runs invoke the
same advisor action as the production client.

The application is deployed on a single VPS. Dokploy runs
...
admin dashboard. The Convex deployment is self-hosted against a
...
setup is deliberately small: the prototype runs on one VPS and
has to run convincingly end to end, not at scale.

\section{Data Layer}
\label{sec:sys-data}

The Convex schema defines twelve tables grouped into four
domains: catalog, transactional, behavior-and-AI, and
...
\begin{figure}[ht]
  ...
  \caption{The schema grouped into four domains.}
  \label{fig:er-diagram}
\end{figure}

Foreign keys flow from \texttt{products} into the
transactional, behavior, and recommendation tables. Anonymous
behavior sessions carry only a \texttt{sessionId} until a
\texttt{clerkUserId} is attached for authenticated traffic.
...
product card render is a single indexed read with no joins, so
grid pages stream from Convex with no intermediate compute. The
table declares nine indices, each tied to a specific
...
DummyJSON feed are extended with faker.js attributes, plus
\texttt{useCases} and \texttt{goodFor} fields from a batch
Flash-Lite call modeled on COSMO~\cite{COSMO}. Roughly 50
...
\code{convex/recommendations.ts} (Convex queries, mutations,
actions) and \code{convex/recommendationHelpers.ts} (pure
...
scores from the \code{productCoOccurrences} table. Computation
...
co-occurrences drops spurious pairs.
...
Section~\ref{sec:found-similarity}. The point is that this is a
12-line function with no external dependencies: at the
prototype's catalog scale, classical content-based similarity is
trivially cheap and enough to fill the ``similar products'' slot
on every product detail page. The \texttt{useCases} and
\texttt{goodFor} fields it reads are produced once per product
during seed by a batch Flash-Lite call~\cite{COSMO}, adding
structured similarity that pure attribute matching would miss.
...
\end{minted}
  \caption{The content-similarity function.}
  \label{lst:content-similarity}
\end{listing}

\paragraph{Trending and personalized.} Trending uses the
exponential-decay formula of Equation~\ref{eq:trending} with
$\lambda \approx 0.0495$
...
Section~\ref{sec:found-classical}. Signals are combined using
Reciprocal Rank Fusion with $k = 60$, followed by Maximal
Marginal Relevance at $\lambda = 0.7$ to force diversity. The
full hybrid is about 80 lines of straight-line code; no library
is involved.

\begin{calloutbox}{Switching-Hybrid Algorithm Selection}
  The algorithm chosen for the personalized slot depends on the
  depth of the user's purchase history:
  \begin{itemize}
    \item \textbf{Cold user} (no purchases): falls through to
      trending. The cold-start problem has no signal-based answer.
    \item \textbf{Lukewarm user} (1--3 purchases): content
      similarity to favorited items, blended with category-scoped
      trending.
    \item \textbf{Warm user} ($\ge 4$ purchases):
      Jaccard co-occurrence against the user's purchase history,
      blended with content similarity over favorites.
  \end{itemize}
\end{calloutbox}
...
rerank step in Section~\ref{sec:sys-llm}. Stage~1 reads candidate
identifiers from the Convex catalog under a tight latency budget.
Stage~2 invokes the LLM over that bounded candidate set only and
emits identifiers plus a free-text rationale; it never reads
the catalog directly. The client then hydrates the cited
identifiers against the live catalog before rendering, so every
field the user sees comes from the catalog and not from the LLM.

\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[
      box/.style={draw, rounded corners=2pt, align=center,
      minimum width=1.9cm, minimum height=0.9cm, font=\small},
      ...
      catalog/.style={draw, rounded corners=3pt, align=center,
        minimum height=0.9cm, minimum width=11cm,
      font=\small\bfseries, fill=blue!6},
      arr/.style={-{Stealth[length=2mm]}, thick},
      darr/.style={-{Stealth[length=2mm]}, thick, dashed, gray!70!black},
      note/.style={font=\footnotesize\itshape, align=center,
      text=gray!60!black},
      lbl/.style={font=\footnotesize\itshape, fill=white, inner sep=1.5pt}
    ]
    % linear data flow (bottom row)
    \node[box] (query) {Slot request \\ or user query};
    \node[stage, right=0.4cm of query] (s1) {Stage~1 \\[2pt]
    \normalfont\small Classical \\ \normalfont\small candidates};
    \node[stage, right=0.4cm of s1] (s2) {Stage~2 \\[2pt]
    \normalfont\small LLM rerank \\ \normalfont\small + explain};
    \node[box, right=0.4cm of s2] (out) {productIds \\ + rationale};
    \node[box, right=0.4cm of out] (hyd) {Client \\ hydration};

    % catalog above, spanning
    \node[catalog, above=1.4cm of s2] (db) {Convex catalog};
    ...
    % catalog lookups: one into Stage 1, one into Client hydration
    \draw[darr] (db.south -| s1.north) -- (s1.north)
    node[lbl, midway] {read};
    \draw[darr] (db.south -| hyd.north) -- (hyd.north)
    node[lbl, midway] {hydrate};

    % per-stage notes
    \node[note, below=0.4cm of s1] {Convex query \\ p95 $<$ 50\,ms};
    \node[note, below=0.4cm of s2] {LLM call, \\ candidate-bounded};
  \end{tikzpicture}
  \caption{The two-stage recommendation pipeline.}
...
\code{apps/web/src/hooks/}. \code{useDwellTime} runs a tick
counter while the cursor is over a product container.
\code{useViewportTracking} accumulates
IntersectionObserver-reported visible time.
\code{useScrollDepth} maintains the page-level maximum scroll
fraction. \code{useProductEngagement} composes the first three
behind a single product-scoped facade.
\code{useBehaviorTracker} aggregates everything into the
per-session buffer that flushes to the backend.

Listing~\ref{lst:use-dwell-time} shows the core of
\code{useDwellTime}. The hook has two effects: one attaches the
\texttt{mouseenter} and \texttt{mouseleave} listeners, and one
runs a 100~ms tick interval while the cursor is hovering. The
cumulative dwell time is the sum of past hover sessions plus the
current tick count, so the consuming component reads the live
value without waiting for a hover to end.
...
\code{useBehaviorTracker} keeps a
...
triggers invoke the flush: a 5-second interval, a
pathname-change effect, and a \code{visibilitychange} listener
...
\begin{figure}[ht]
  ...
  \caption{Behavior aggregation and flush pipeline.}
...
\code{useReadinessSignals} hook reads the in-memory behavior
state and returns \code{shouldPulseAdvisor} and
\code{activeChips}; dismissed chips stay dismissed for the
session. A composite interest score over dwell, scroll, hover,
and review-tab engagement is also computed and kept as a
context-richness hint for the advisor prompt. The simpler
per-signal thresholds were easier to interpret during early
internal use, so they currently drive the indicators. The
...
\code{@convex-dev/agent}, the official Convex agent component.
...
\code{openai/gpt-oss-120b}, served by Cerebras with Groq as
fallback, at medium reasoning effort. The batch tier is
\code{google/gemini-3.1-flash-lite-preview} with reasoning
disabled. Listing~\ref{lst:select-model} shows
\code{selectModel} from \code{convex/ai/models.ts}: the model
map resolves a query type to a model identifier, and for gpt-oss
models, provider routing prefers Cerebras then Groq for
throughput.

\begin{calloutbox}{Model Routing by Query Type}
  \textbf{Reasoning tier} (\texttt{gpt-oss-120b}, Cerebras with
  Groq fallback): conversational, comparison, personalized advice.
  \textbf{Batch tier} (\code{gemini-3.1-flash-lite-preview},
  reasoning disabled): review summary, quick Q\&A, homepage hero.
\end{calloutbox}
...
\end{minted}
  \caption{The model-routing function.}
  \label{lst:select-model}
\end{listing}

\paragraph{Agent, tools, and prompt.} The agent is created
...
\code{hasFinalAnswer} scorer was added to catch automatically.
Five tools are registered: \code{getProductDetails},
\code{searchProducts}, \code{getRecommendations} (which
dispatches to similar, frequently-bought-together, or trending),
\code{getCartContents}, and \code{getReviewsSummary} (which
...
covered by a modal). Four components compose it under
\code{components/advisor/}: the floating button with the
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{advisor-request.pdf}
  \caption{End-to-end advisor request, from user click to
  hydrated render.}
...
ever generated on a user-facing critical path. The
conflict-surfacing structure is the anti-Rufus design choice from
Chapter~\ref{chap:background}: ``45 reviews praise battery life,
12 reviews call it disappointing'' is more useful than ``most
users are happy with the battery''.

\begin{calloutbox}{Review-Summary Schema}
  Four fields per summary: \textbf{what buyers like} (themes with
  counts and quotes), \textbf{common complaints} (same structure,
  opposite sentiment), \textbf{best for} (buyer cohorts), and
  \textbf{conflicting opinions} (disagreement pairs with per-side
  counts).
\end{calloutbox}
...
representative quote and the source reviewId. After
...
are dropped before persistence; the \code{validationDetails}
...
themes, and no winner is named; the decision stays with the
user. This deliberately departs from how most shopping AI handles
comparison; the related-work survey in
Section~\ref{sec:bg-comparison} reaches the same
...
Output validation runs at the boundary between the agent action
and the client response, inside the same Convex action that
wraps the agent call. The three validators run
sequentially as formalized in
Section~\ref{sec:found-grounding}. Figure~\ref{fig:validation-pipeline}
shows the pipeline.

\begin{calloutbox}{Three Sequential Validation Checks}
  \begin{enumerate}
    \item \textbf{Identifier-existence.} Every cited productId is
      intersected with the live catalog. Identifiers that do not
      exist are stripped silently before the action returns.
    \item \textbf{Claim stripping.} A regex pass removes
      currency-prefixed numbers and unit-suffixed numbers
      (\texttt{GB}, \texttt{Hz}, \texttt{mAh}) from the free-text
      rationale. The system prompt already forbids these claims;
      this is the second-line defense.
    \item \textbf{Theme fidelity.} For review-summary outputs,
      each claimed theme is embedded and matched against the
      source review corpus by cosine similarity. Themes below the
      threshold $\tau$ are dropped before persistence.
  \end{enumerate}
\end{calloutbox}
...
\begin{figure}[ht]
  ...
  \caption{The output validation pipeline.}
...
forbids these claims; the regex pass is the second line of
defense when the model fails to follow the prompt.

\paragraph{Theme verification.} Verification only applies to
outputs that include claimed review themes (the review-summary
  surface and some advisor responses that quote review
feedback). The verifier reuses the embedding search from
Section~\ref{sec:sys-reviews}: each claimed theme is
...
Two surfaces show the visual contract the system makes with
the user. The first is the homepage with its recommendation
...
\begin{figure}[ht]
  ...
  \caption{The homepage surface.}
  \label{fig:ui-homepage}
\end{figure}

The homepage is built entirely from classical recommendation
slots: a popularity row on top, the personalized
``Recommended for you'' switching-hybrid output below. None of
these calls an LLM. The entire homepage renders from
...
\begin{figure}[ht]
  ...
  \caption{The AI-generated review summary on the product
  detail page.}
  \label{fig:ui-review-summary}
\end{figure}

The review-summary surface brings together the anti-Rufus
design choices. The explicit positives-and-complaints split
shows each theme with a per-theme count and a representative
quote. The bipartite conflicts bar shows divided opinions with
agreeing- and disagreeing-review counts. The ``Best for'' pills
display the buyer-cohort field. A small ``AI-generated'' tag
discloses the synthetic origin. The contextual
``What do buyers think?'' chip is the review-engagement
readiness signal: it sends a question to the advisor on click
but never triggers an LLM call by itself. Neither surface shows
a number, attribute, or price that comes from the LLM rather
than from the live catalog.

\section{Evaluation Harness}
\label{sec:sys-eval}

The eval harness lives in \code{packages/eval} and is built
...
Convex action wrapping the production advisor agent.
Programmatic, built-in, and LLM-as-judge scorers all write into
a single JSON results file per sweep.

\paragraph{Build vs.\ buy.} Of the engineering effort, only the
scoring is distinctive to this thesis: whether an answer cited a
real product, whether a
...
\code{src/datasets/shopping-v1.yaml}, a 25-row YAML file with
each row carrying the question, a simulated context
(\code{productId}, \code{recentlyViewedIds}), the expected
tools the agent should call, and the productIds the answer
should reference. Nine rows appear twice with different expected
tools, so one sweep runs 34 test cases per configuration. Rows
are distributed across seven categories
(Table~\ref{tab:eval-dataset-categories}). Dataset changes are
versioned by filename, so old runs are never compared against
new questions.
...
\paragraph{Configuration sweep.} Eight configurations are
...
($8 \times 34 = 272$ advisor invocations and roughly
1\,100 LLM-judge calls).
...
\begin{table}[ht]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{|l|l|c|c|l|}
      ...
      \hline
      baseline-flash-lite      & gemini-3.1-flash-lite-preview & n/a
      & 12 & current \\
      ...
      medium & 12 & current \\
      oss-120b-tight           & gpt-oss-120b                  &
      medium & 8  & ``be efficient'' \\
      oss-120b-loose           & gpt-oss-120b                  &
      medium & 15 & current \\
      oss-120b-no-fewshot      & gpt-oss-120b                  &
      medium & 12 & no few-shot \\
      \hline
    \end{tabular}%
  }
  \caption{The canonical configuration sweep.}
  \label{tab:eval-sweep-configs}
\end{table}

\paragraph{Scorers.} Six programmatic scorers are registered:
\code{hasFinalAnswer}, \code{groundedness},
\code{factuality}, \code{reviewThemeFidelity},
\code{toolCallEfficiency}, and \code{expectedToolCoverage}
(an F1 score between the tools the agent called and the tools
the row expected). Listing~\ref{lst:groundedness}
shows \texttt{groundedness.ts} in full. It is the scorer most
directly aimed at the Rufus 32\% accuracy failure mode. Convex
product IDs are 32-character URL-safe strings that start with a
lowercase \texttt{k}, which the regex matches before each cited
identifier is looked up against the live catalog.
...
\end{minted}
  \caption{The \texttt{groundedness} scorer.}
...
Claude Haiku~4.5 as the judge model, chosen from a different
model family to reduce self-preference bias, as formalized in
Section~\ref{sec:found-eval}.

\section{Results}
\label{sec:sys-results}

The headline finding is short: cheaper and more conservative
configurations win on composite; aggressive-reasoning ones lose.
The rest of the section reports the numbers.

The composite score combines judge quality (weight 0.30),
...
full 34 test cases of the dataset (25 questions, nine of them in
two expected-tool variants), so row coverage is uniform across
cells.
...
\begin{table}[ht]
  ...
  \caption{Configurations ranked by composite score.}
  \label{tab:results-ranked-configs}
\end{table}

Three observations follow. The Gemini~3.1 Flash-Lite baseline
ranks \#1 on composite (0.807) even though the original design
treated it as a cheaper fallback. It loses on raw quality
by 3.9 points to \texttt{no-fewshot} \texttt{gpt-oss-120b}
(0.776 vs.\ 0.815) but wins clearly on both penalty axes:
...
despite competitive raw quality (cost doubles, p95 rises to
...
\begin{figure}[ht]
  ...
  \caption{The cost-quality Pareto front.}
...
\begin{figure}[ht]
  ...
  \caption{The latency-quality Pareto front.}
...
configuration lies in the strictly-dominated interior. For
production, the only choice that has to be made on this
evidence is between Flash-Lite and \texttt{no-fewshot}. The
latency-quality plane tells the same structural story.
Flash-Lite and \texttt{no-fewshot} again anchor the frontier,
with Flash-Lite cheaper \emph{and} faster (4.2~s p95 versus
5.9~s) at the cost of those 3.9 quality points.
...
\begin{table}[ht]
  ...
  \caption{Mean programmatic correctness per category and
  configuration.}
...
the product in the row's system context. Dropping the few-shot
examples removes the contamination.

The failure is clearest on a single row. The dataset row asks
for the price of a Vivo~V9; the row's system context names the
product explicitly.
...
The mechanism is simple: few-shot examples are injected into
...
within 0.3~pp. A focused two-configuration A/B from earlier
development ranked the variants identically on all four judge
rubrics. Two scoring methods agreeing on the same ordering
argues the finding is not an artifact of either.

\subsection{Ablation: Removing Grounding}
\label{subsec:results-ablation}

The sweep compares configurations that all share the grounded
architecture. To measure what the architecture itself buys, a
ninth configuration was run separately with the grounding
removed. It keeps the same model, reasoning effort, step budget,
few-shot examples, and system context as
\code{gpt-oss-120b-medium}, but the agent gets no tools, and
its prompt tells it to answer from the conversation context and
its own knowledge. This is what a chatbot without catalog access
looks like, and it is the direct-generation pattern that
Chapter~\ref{chap:background} identified as the root cause of
the Rufus failures. Table~\ref{tab:results-ablation} compares
the two configurations on the same 34 test cases.
...
\begin{table}[ht]
  ...
  \begin{tabular}{lcc}
    ...
    & Grounded & Ungrounded \\
    \midrule
    specific price and rating claims & 83 & 162 \\
    claims not matching the store database & 48\% & 67\% \\
    ...
    \bottomrule
  \end{tabular}
  \caption{The grounded \texttt{gpt-oss-120b-medium}
  configuration against the ungrounded ablation.}
  \label{tab:results-ablation}
\end{table}

Three findings follow.

First, without tools the model answers anyway, and makes twice
as many specific claims: 162 price and rating claims against 83,
with 67\% failing the database check against 48\%. The grounded
mismatches are mostly a coverage artifact (the snapshot only
holds the products the row referenced, so a correct mention of
any other catalog product counts against it). The ungrounded
mismatches are fabrication. One row asks for the price of the
phone the user is viewing. The ungrounded answer is a confident
price table quoting the Samsung Galaxy S24 Ultra at \$1,199 and
the iPhone 15 Pro at \$999: products pulled from the few-shot
examples, priced from the model's memory of the real world, and
none of it from the store. Every claim in that row fails the
check, and the table format makes the answer look authoritative.
The identifier check does not discriminate here (both
configurations score 1.0 on \texttt{groundedness}) because the
ungrounded model only repeats the one identifier it got from
context and invents none. The fabrication shows up in prices and
ratings, not in identifier syntax.

Second, the LLM judges prefer the fabricating configuration:
helpfulness 0.78 against 0.57, completeness 0.89 against 0.70.
The judges see only the question and the answer, and a specific,
confident answer reads better than a careful grounded one. They
reward exactly the failure the architecture exists to prevent.
This is the strongest argument in the thesis for the harness
design of Section~\ref{sec:sys-eval}: programmatic checks
against the live database carry the trust signal, and judge
scores are a secondary quality signal, never the gate.

Third, grounding has a latency price: p50 of 2.4~s against
1.4~s. The ungrounded model answers in a single call; the
grounded one pays for tool round-trips. The architecture spends
about one second per query to make its answers checkable.

In production, the validation pipeline of
Section~\ref{sec:sys-validation} sits after the model, strips
unverifiable claims from free text, and drops unknown
identifiers, so the user never sees most of what the ungrounded
configuration would say. The ablation measures the raw model
output before that layer, which is the right place to measure:
it shows the architecture does real work, rather than guarding
against a failure that would not happen anyway.

\subsection{Summary and Validity}
\label{subsec:results-summary}

The evidence supports the central claim on two of three axes.
Every configuration completes well under the consumer-tolerance
latency bound and at well under one cent per query, with
grounded output across the sweep (\texttt{groundedness} mean
$\ge 0.89$). The harness adds a finding the claim did not
anticipate: a non-trivial design space exists \emph{within}
those bounds where the right choice is cheaper and less
computationally aggressive rather than the one with the highest
raw quality. Higher reasoning effort and looser step budgets
are uniformly worse on composite, which disproves the implicit
``more reasoning is always better'' assumption.

Three validity classes qualify these numbers.

\textbf{Internal validity.} Seeded reviews are AI-generated, and
residual judge bias remains despite the cross-family judge
choice.

\textbf{Construct validity.} Groundedness verifies existence,
not relevance. Factuality verifies values, not semantic
correctness. Theme fidelity verifies embedding match, not
salience. The composite is reported alongside the Pareto fronts,
so disagreement with the weights is recoverable.

\textbf{External validity.} 200 products on a development-tier
deployment do not predict 10\,000-product production load, and
the 25-question dataset cannot cover every failure mode. The
methodological finding (programmatic and cross-family judges
agree on ordinal ranking) argues that the harness produces a robust
ranking even when individual scorers are imperfect.

\section{Engineering Challenges}
\label{sec:sys-engineering}

Six engineering challenges met during implementation justify
choices that affect the metrics above. Each is documented at
length in \code{docs/lessons-learned.md}.
...
\begin{itemize}
  ...
  \item \textbf{Hydration mismatches with Base UI.} Components
    ...
    \code{useSyncExternalStore}, returning \texttt{false} from
    the server snapshot to collapse the typical two-render
    pattern into one.
  ...
  \item \textbf{\texttt{useUIMessages} typing.} The message type
    is derived from \code{api.ai.queries.listMessages} via
    \texttt{UIMessagesQueryResult<typeof ...>}, avoiding a
    hand-duplicated type.
  \item \textbf{\texttt{content-visibility} grid
    virtualization.} The CSS-native \code{content-visibility}
    property, set to \texttt{auto} and paired with
    \code{contain-intrinsic-size}, virtualizes 50--150-card
    grids without a JavaScript library.
