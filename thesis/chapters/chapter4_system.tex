...
\texttt{google/gemini-3.1-flash-
lite-preview} with reasoning
...
\begin{calloutbox}{Model Routing by Query Type}
  ...
  \textbf{Batch tier} (\texttt{gemini-3.1-flash-lite-preview},
...
ever generated on a user-facing critical path. The conflict-
surfacing structure is the anti-Rufus design choice from
...
($8 \times 25 = 200$ advisor invocations).
...
\begin{table}[ht]
  ...
  \caption{Configuration sweep for the canonical evaluation.
    ...
    producing 200 advisor invocations and approximately 1\,000
  LLM-judge calls.}
...
the 25-row dataset cannot cover every failure mode. The
...
\begin{itemize}
  ...
  \item \textbf{Hydration mismatches with Base UI.} Components
    ...
    \texttt{useSyncExternalStore}, returning \texttt{false} from
  ...
  \item \textbf{\texttt{useUIMessages} typing.} The message type
    is derived from \texttt{api.ai.querie-
    s.listMessages} via
    \texttt{UIMessagesQueryResult<typeof ...>}, avoiding a hand-
    duplicated type.
  \item \textbf{\texttt{content-visibility} grid
    virtualization.} The CSS-native
    \texttt{content-visi-
    bility: auto} property paired with
    \texttt{contain-intrinsic-size} virtualizes 50--150-card
