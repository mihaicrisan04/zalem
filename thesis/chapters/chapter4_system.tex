...
\begin{figure}[ht]
  ...
  \end{tikzpicture}
  \caption{Two-stage recommendation pipeline: classical
    candidate retrieval, LLM rerank-and-explain, and client-side
  hydration against the live catalog.}
...
($8 \times 34 = 272$ advisor invocations).
...
\begin{table}[ht]
  ...
  \caption{Configuration sweep for the canonical evaluation.
    Each configuration is run against all 34 test cases,
    producing 272 advisor invocations and approximately 1\,100
  LLM-judge calls.}
...
\begin{table}[ht]
  ...
  \caption{Configurations ranked by composite score. Each row is
    a single configuration evaluated against the 34 test cases of
...
\begin{table}[ht]
  ...
  \caption{The grounded \texttt{gpt-oss-120b-medium}
    configuration against the ungrounded ablation on the same 34
  test cases.}
