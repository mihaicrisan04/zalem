...
configurations on grounded quality, latency, and cost across an
eight-configuration canonical sweep.
...
inflates to 12.1~s. Every configuration costs well under one cent
per query and produces grounded output across the sweep
(\texttt{groundedness} mean $\ge 0.89$ for every configuration).
The harness adds a finding the claim did not anticipate: there is
a real design space within those bounds where the right choice is
the cheaper, less computationally aggressive configuration rather
than the one with the highest raw quality. Higher reasoning
effort and looser step budgets are uniformly worse on composite,
which disproves the implicit ``more reasoning is always better''
assumption that production code tends to bake in.
...
(about 200 products, 4\,836 AI-generated reviews), and the
single-deployment replication horizon. The thesis does not claim
to address any of these; it claims that the engineering work it
does report holds up under measurement within those limits.
