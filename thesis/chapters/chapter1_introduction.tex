...
substitutes, and shoppers spend considerable time browsing,
comparing, and checking products before they buy. Recommender systems
...
one limitation that affects the user directly: the output is
opaque.
...
write natural-language explanations, summarize customer reviews,
...
Consumer surveys support this: 55\% of users say they do not
...
AI suggestions offered after a natural pause are better received
than proactive interruptions~\cite{CHI2024Timing}.

The open question is not whether to put LLMs into e-commerce,
but how. The architecture and interaction choices decide whether
the assistant is trusted, accurate, and welcome. At
bachelor-thesis scale, they have not been studied systematically
against a documented production system. This thesis proposes
both: an architecture that grounds every factual claim in the
live catalog, and a process that backs every configuration
choice with measured evidence. Both are validated in a working
prototype.
...
natural pause read as helpful. Suggestions injected while the
user is still deciding read as
...
be welcome, but it never triggers AI advice automatically. The
user must engage deliberately. Behavior signals prepare context. The user
pulls.

Amazon's own product timeline points the same way. Rufus
launched as a reactive assistant. The proactive ``Help Me Decide'' surface was
...
The central claim of this thesis has two parts. A hybrid
architecture that combines classical recommender algorithms with
a reactive, grounded LLM advisor can answer real shopping
questions without inheriting the trust failures of production
systems. Every model and prompt choice in such an architecture
can be backed by measured evidence rather than intuition.

\section{Objectives}
\label{sec:intro-objectives}

This thesis proposes an architecture for grounded, LLM-augmented
shopping recommendation and a process for configuring it on
measured evidence. Both are realized in a working prototype that
...
\begin{enumerate}
  ...
  \item Build a review summarization flow that surfaces
    ...
    most criticized in similar production systems.
...
Chapter~\ref{chap:foundations} introduces the concepts the
prototype builds on: similarity measures (Jaccard, weighted
attribute scoring, cosine, exponential time decay), the two-stage
retrieve-and-rerank pattern, the LLM-agent loop with tool use
and structured output, the hydration-over-generation principle as
a structural defense against hallucination, and the evaluation
metrics used to compare configurations.

Chapter~\ref{chap:system} documents Zalem, the prototype store
built for this thesis, as a case study. It opens with
...
and the measured results across an eight-configuration sweep,
including Pareto fronts on cost-quality and latency-quality, a
composite ranking that justifies the shipped configuration, and
an ungrounded ablation baseline that measures what the grounded
architecture buys.

Chapter~\ref{chap:conclusions} summarizes what the prototype
shows, what the measured evaluation supports, and the
