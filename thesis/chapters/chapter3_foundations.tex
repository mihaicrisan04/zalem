\chapter{Fundamentals of Recommendation and an Approach for
Grounded Output}
\label{chap:foundations}

This chapter lays out the foundations the prototype rests on and
the design principles this thesis contributes. It keeps the two
apart on purpose. The similarity measures, the
retrieve-and-rerank pattern, and the agent loop are established
ideas, used here as building blocks and cited to their sources.
Hydration over generation and the validation pipeline are this
thesis's own response to the failure modes from
Chapter~\ref{chap:background}. Each section defines a concept,
states the property the prototype relies on, and leaves the
implementation detail for Chapter~\ref{chap:system}.

Section~\ref{sec:found-classical} recaps the classical
recommender families. Section~\ref{sec:found-similarity} defines
the similarity measures and the time-decay function used
throughout the system. Section~\ref{sec:found-two-stage} states
the two-stage retrieve-and-rerank pattern.
Section~\ref{sec:found-agents} describes the LLM-agent loop with
tool use and structured output. These four are standard.
Section~\ref{sec:found-grounding} then presents hydration over
generation and the post-generation validation pipeline, the
structural defense against hallucination that this thesis
contributes. Section~\ref{sec:found-eval} defines the evaluation
metrics used to compare model and prompt configurations.
...
set is reproducible across evaluation runs). Every algorithm
...
\paragraph{Jaccard similarity.} Jaccard
similarity~\cite{Jaccard1912} measures the share of elements two
sets have in common. For two finite sets $A$ and $B$,
\begin{equation}
  J(A, B) \;=\; \frac{|A \cap B|}{|A \cup B|},
  \label{eq:jaccard}
\end{equation}
where two empty sets score zero by convention. The coefficient
is symmetric, takes values in $[0, 1]$, and equals one exactly
when $A = B$.
...
prototype uses Equation~\ref{eq:jaccard} over buyer sets for the
``frequently bought together'' slot. A minimum-support filter
requires $|B(p_a) \cap B(p_b)| \ge 2$ for a pair to enter the
precomputed co-occurrence index. The filter removes spurious
links from chance co-purchases.
...
LLM-enriched fields. Formally, the score is a weighted sum of
attribute similarities, the standard form in content-based
filtering~\cite{ComprehensiveRecSys2024}:
\begin{equation}
...
coefficient $J(\cdot, \cdot)$ of Equation~\ref{eq:jaccard} over a
tag-set argument, or a
proximity score $\mathrm{prox}(x, y) = 1 - |x-y|/\max(x,y)$ for
numeric attributes, which equals one when the two values match
and falls toward zero as they move apart.
Table~\ref{tab:found-sim-weights} lists the
...
\begin{table}[ht]
  ...
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{Components of the weighted attribute similarity
  score.}
...
they are orthogonal (the vectors share no
direction)~\cite{ManningIR2008}. For two vectors
$u, v \in \mathbb{R}^d$,
\begin{equation}
  \cos(u, v) \;=\; \frac{\langle u, v \rangle}{\|u\| \cdot \|v\|}.
  \label{eq:cosine}
\end{equation}
The prototype uses Equation~\ref{eq:cosine} over text embeddings
only inside the
review-theme-fidelity scorer of Section~\ref{sec:found-eval}.
Each candidate review-summary theme is embedded and compared
against the embeddings of the source reviews. Themes whose
nearest review falls below a threshold $\tau$ are rejected.
Cosine is preferred over Euclidean distance here because
embedding norms carry no meaning.
...
than a sale from a month ago. This recency weighting is standard
in session-based recommendation~\cite{SessionRecSys2021}. For a
product $p$ with event set
...
at query time. The half-life is the only meaningful parameter, and it
...
layer to the LLM layer. It comes from information retrieval,
from cascade ranking in web search~\cite{HSTU2024} through to
production e-commerce deployments~\cite{ShopifyHSTU}. The thesis
adopts it for one reason. It is the structural answer to
the question Chapter~\ref{chap:background} raised about Rufus:
how an LLM-assisted shopping system avoids hallucinating
products and specifications.

Formally, following the cascade-ranking
formulation~\cite{HSTU2024}, let $\mathcal{C}$ be the full
product catalog and $q$ a
slot intent or user query. A retrieve function $R$ produces a
small candidate set
\begin{equation}
  S \;=\; R(q, \mathcal{C}), \quad |S| \ll |\mathcal{C}|,
  \quad S \subseteq \mathcal{C},
  \label{eq:retrieve}
\end{equation}
under a tight latency budget $\tau_R$. In words, the retrieve
stage narrows the full catalog down to a small set of real
catalog products before the LLM is involved. A rerank-and-explain
...
construction of Equations~\ref{eq:retrieve} and~\ref{eq:rerank},
$\hat{S} \subseteq S \subseteq \mathcal{C}$, so every
...
($|\mathcal{C}| \approx 200$) the second property is easy to
meet; the bound on $|S|$ is what keeps the pattern viable as
$|\mathcal{C}|$ grows.
...
tradeoff explanation, and only ever sees the candidate set plus
...
deployments. A single iteration produces a
triple~\cite{ReAct2022}
\begin{equation}
  (\mathrm{thought}_t, \mathrm{action}_t, \mathrm{observation}_t),
  \label{eq:react-triple}
\end{equation}
...
\code{getProductDetails}, \texttt{searchProducts},
\code{getRecommendations}, \texttt{getCartContents},
\code{getReviewSummary}, \texttt{compareProducts}, and
\code{getCategoryStructure}. Each tool is a thin wrapper over a
...
response must validate against a JSON schema. Native JSON-mode
...
failed turn and is not shown to the user.
...
$B$?) and the \code{toolCallEfficiency} property (how many of
the calls were duplicates?). Any regression on either shows
...
a shopping context, the failure modes listed in
...
generative behavior that produces these failures is inherent to
the model class~\cite{HallucinationSurvey2023}; mitigation must
be structural.

\paragraph{Hydration over generation.} The prototype enforces a
single principle, which this thesis defines as hydration over
generation: the LLM emits structured references, and the
client renders live data from the catalog. Grounding generation
in source data is the standard structural mitigation for
hallucination~\cite{HallucinationSurvey2023}. Formally, let $\hat{S}$
...
removes price and specification claims (defined below). In words,
the user sees the live catalog record for every cited identifier,
plus the cleaned rationale text. The LLM
...
backed by a post-generation validation pipeline designed for
this thesis, with three sequential checks; the definitions below
are the thesis's own. Let $\mathcal{C}_{id}$ denote the set of
...
\begin{enumerate}
  \item \textbf{Identifier-existence.} The structured part of the
    ...
    \begin{equation}
      ...
      \label{eq:id-existence}
    \end{equation}
    Cited identifiers that do not exist in the catalog are dropped
    ...
    main defense against the 32\% recommendation accuracy
  ...
  \item \textbf{Claim stripping.} The free-text rationale $r$ is
    ...
    place; stripping is a second layer that catches lapses in
    prompt following~\cite{ShopifyAgentic}.
  \item \textbf{Theme fidelity (batch path).} For review summaries
    ...
    a nearest-neighbor lookup under the cosine similarity of
    Equation~\ref{eq:cosine}~\cite{ManningIR2008}. A theme is
    accepted exactly when
    \begin{equation}
      \max_{e \in \mathcal{E}_{\mathrm{rev}}} \cos(\mathrm{embed}(t), e)
      \;\geq\; \tau,
      \label{eq:theme-fidelity}
    \end{equation}
    with threshold $\tau$ tuned on a small held-out batch. In
    words, a theme is kept only when at least one real review is
    close enough to it. Themes
...
three layers of scorers, each addressing a different failure
mode.
...
\begin{itemize}
  ...
  \item \texttt{groundedness}: is every productId cited in the
    answer in the catalog? Scored as the share of cited productIds
    that exist in the catalog, formally
    $|\hat{S} \cap \mathcal{C}_{id}| / |\hat{S}|$, so higher is
    better.
  ...
  \item \code{reviewThemeFidelity}: do claimed review themes
  ...
  \item \code{toolCallEfficiency}: a duplicate-call penalty
...
$(\mathrm{lat}, Q)$. The front is the natural way to choose
a deployment configuration under an explicit budget.

A composite score collapses the measurement vector into a single
number through a weighted average across the five axes, the
standard weighted-sum method for multi-objective
ranking~\cite{MarlerArora2010}. It is used
...
Chapter~\ref{chap:system} moves from the abstract architecture
to the concrete prototype:
