\chapter{Hybrid Recommendation with Grounded LLM Assistance}
...
\paragraph{Jaccard similarity.} For two finite sets $A$ and $B$ the
Jaccard coefficient is
\[
  J(A, B) \;=\; \frac{|A \cap B|}{|A \cup B|},
\]
with $J(\emptyset, \emptyset) := 0$ by convention. The coefficient
is symmetric, lives in $[0, 1]$, and equals one iff $A = B$. Used
over the buyer sets of two products $p_a, p_b$ with buyer sets
$B(p_a), B(p_b)$, it produces a similarity score that is robust
to popularity differences: a product with many buyers does not
automatically score high against another just because their
intersection is large~\cite{ComprehensiveRecSys2024}. The
...
\paragraph{Weighted attribute similarity.} For two products
$p_a, p_b$ described by attribute vectors over category, tag set,
brand, price, rating, and the LLM-enriched fields
\texttt{useCases} and \texttt{goodFor}, the prototype defines a
weighted similarity score
\begin{align*}
  \mathrm{sim}(p_a, p_b) \;=\;&
    w_1 \cdot \mathbb{1}[\textrm{same category}] +
    w_2 \cdot J(\textrm{tags}_a, \textrm{tags}_b) \\
    &{} + w_3 \cdot J(\textrm{useCases}_a, \textrm{useCases}_b) +
        w_4 \cdot J(\textrm{goodFor}_a, \textrm{goodFor}_b) \\
    &{} + w_5 \cdot \mathbb{1}[\textrm{same brand}] +
        w_6 \cdot \mathrm{prox}(\textrm{price}_a, \textrm{price}_b) \\
    &{} + w_7 \cdot \mathrm{prox}(\textrm{rating}_a, \textrm{rating}_b),
\end{align*}
where $\mathbb{1}[\cdot]$ is the indicator, $J(\cdot, \cdot)$ is
Jaccard over the tag-set arguments, and the proximity function
$\mathrm{prox}(x, y) = 1 - |x - y|/\max(x, y)$ is one minus the
normalized absolute difference. The weights $w_1, \dots, w_7$ are
fixed at $(0.25, 0.20, 0.15, 0.10, 0.10, 0.10, 0.10)$; the choice
favors category and tag overlap and gives each remaining feature
an equal small contribution. The \texttt{useCases} and
\texttt{goodFor} fields carry implicit attributes (a children's
bicycle is also ``a gift for a 7-year-old'') and are produced
once per product during seed by a batch LLM call modeled on the
COSMO knowledge graph~\cite{COSMO}. At query time, $\mathrm{sim}$
is pure attribute arithmetic over the denormalized product row.

\paragraph{Cosine similarity.} For two vectors $u, v \in
\mathbb{R}^d$ the cosine similarity is
\[
...
\paragraph{Exponential time decay.} For a product $p$ with event
set $E_p$, per-event weight $w(\cdot)$ (a purchase weighs more
than a view), and event age $\mathrm{age}(e)$ measured in days
from now, the trending score is
\[
...
A composite score collapses the vector into a scalar for
ranking:
\[
  \mathrm{composite}(c) \;=\;
  0.30 \cdot Q(c) +
