...
- the time saved (~3–5 weeks of webapp engineering reduced to ~3–5 days of glue code) goes into more dataset coverage and writing
...
- **chapter 7 (results closing section):** discuss the limits of the harness — small dataset, judge biases, seeded-data limitations.
...
the harness measures system properties only: correctness, cost, latency, judge-perceived quality. user-perceptible properties (trust, intrusiveness, control) are out of scope for this thesis per the 2026-05-13 direction change recorded in `docs/PLAN.md`. human-subject evaluation is acknowledged as future work in chapter 8.
...
- **seeded catalogue is small** — mitigate by noting this in chapter 6 validity threats and including hard-lookup edge cases in the dataset
...
- **the runner replicates production code paths but does not replicate user behavior** — the harness explicitly does not claim to predict UX outcomes; closing that gap requires the human-subject evaluation that is acknowledged as future work in chapter 8
